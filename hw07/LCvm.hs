...
-- For Homework 7 ------ PROBLEM 2 A ---------------------------------------------
        DUP   -> (pc+1, sp', stk',regs)
              where (sp',stk') = if sp == 0
                                    then (0,stk)
                                    else (sp+1,set stk (sp) top)
...
-- For Homework 7 ------------------ PROBLEM 1 A ---------------------------------
        INC   -> (pc+1, sp, set stk (sp-1) v             , regs)
            where v = mod (top+1) 255

        DEC   -> (pc+1, sp, set stk (sp-1) v             , regs)
            where v = mod (top-1) 255
----------------------------------------------------------------------------------
...
-- The value x1 in stack is 0 because after push 254 it is incremented
-- and reaches 255, which is overflow, then it turns 0.
myIncTest1 = stepRun [Push 254, Inc, Halt]

-- x1's value is 13 because it's pushed 10, then incremented 3 times.
myIncTest2 = stepRun [Push 10, Inc, Inc, Inc, Halt]

-- x1's value is 0. 1 is pushed the decreased two times. It would be -1 but our
-- aritmetic limit of our language is 0-255. That way, the final value is 0.
myDecTest1 = stepRun [Push 1, Dec, Dec, Halt]

-- x1's value is 13. 16 is pushed then decreased 3 times. 
myDecTest2 = stepRun [Push 16, Dec, Dec, Dec, Halt]
---------------------------------------------------------

--- TESTS FOR PROBLEM 2 b)

-- 13 will be duplicated at the top of the stack
myDupTest1 = stepRun ([Push x | x <- [1,2,13]] ++ [Dup, Halt])
---------------------------------------------------------
