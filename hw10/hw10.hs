...
------------- THE PROBLEMS ARE COMMENTED FOR REASONS OF TESTING ----------
...
{-
eval env (Var x,s)     = eval env' (e,s1)
                         where (Closure e env',s1) = (elookup x env, s)
                         
eval env (Fun x e,s)   = (Closure (Fun x e) env,s)

eval env (App e1 e2,s) = eval (extend env' x (Closure e2 env)) (e1', s1)
                         where
                              (Closure (Fun x e1') env',s1) = eval env (e1,s)
-}
